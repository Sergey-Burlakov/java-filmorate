...
public class FilmDbStorage extends BaseRepository<Film> implements FilmStorage {
    ...
    private static final String FIND_BY_ID_QUERY = "SELECT f.*, m.name AS mpa_name FROM films AS f JOIN mpa AS m ON" +
            " f.mpa_id = m.id WHERE f.id = ?";
    private static final String FIND_ALL_QUERY = "SELECT f.*, m.name AS mpa_name FROM films AS f JOIN mpa AS m ON" +
            " f.mpa_id = m.id";
    private static final String FIND_GENRES_BY_FILM = "SELECT * FROM   genres AS g JOIN film_genres AS fg ON g.id =" +
            " fg.genre_id WHERE  fg.film_id = ? ORDER BY g.id ASC";
    private static final String GET_TOP_POPULAR_QUERY = "SELECT f.*, m.name AS mpa_name FROM films AS f " +
            "JOIN mpa AS m ON f.mpa_id = m.id " +
            "LEFT JOIN (SELECT id_film, COUNT(id_user) AS likes_count FROM film_likes GROUP BY id_film)" +
            " AS likes_rating ON f.id = likes_rating.id_film " +
            "ORDER BY COALESCE(likes_rating.likes_count, 0) DESC LIMIT ?";
